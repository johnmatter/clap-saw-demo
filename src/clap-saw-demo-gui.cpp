...
ClapSawDemoGUI::ClapSawDemoGUI(ClapSawDemo* processor)
  ...
  setFixedAspectRatio({10, 4});  // 10x6 grid for our layout
}
...
void ClapSawDemoGUI::makeWidgets() {
  ...
  _view->_widgets.add_unique<DialBasic>("f0", ml::WithValues{
    ...
    {"log", true},
    {"visible", true},
    {"draw_number", false},  // TODO: fix font issue
    {"param", "f0"}         // Links widget to parameter (gets default from parameter tree)
  });
  ...
  _view->_widgets.add_unique<DialBasic>("Q", ml::WithValues{
    ...
    {"param", "Q"}          // Links widget to parameter (gets default from parameter tree)
  });
...
void ClapSawDemoGUI::connectParameters() {
  ...
  if (processor) {
    ...
    for (const auto& paramDesc : processor->getParameterTree().descriptions) {
      ...
      std::string logMsg = "Initial sync: " + paramName + " = " + std::to_string(normalizedValue);
...
void ClapSawDemoGUI::render(NativeDrawContext* nvg) {
  ...
  // // Log render calls to see if rendering is happening
  ...
  if (processor && msg.address) {
    ...
    if (addr.getSize() > 2 && second(addr) == "set_param") {
      ...
      if (processor) {
        ...
        processor->logToHost(0, "Requested host parameter flush");
      }
    }
  }
...
void ClapSawDemoGUI::animate(NativeDrawContext* nvg) {
  ...
  // TODO: make this more consistent with other platforms
  _handleGUIEvents();
